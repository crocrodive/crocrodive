<div class="h-full w-full flex flex-col overflow-y-hidden">
    ...
    <div class="flex flex-col items-center  w-full gap-8 p-8 overflow-y-scroll">
      ...
      bg-background-100 hover:shadow-buttonBlack transition-all duration-300'>
        ...
        <div class=" grid-cols-3 grid gap-8">
            @foreach ($skills as $skill)
                <x-card wire:click.prevent="openUpdateSkillDialog({{ json_encode($skill) }})">
                    <div class="w-[300px] h-[88px] flex justify-center items-center text-ellipsis p-4 cursor-pointer">
                        {{$skill['skil_label']}}
                    </div>
                </x-card>
            @endforeach
